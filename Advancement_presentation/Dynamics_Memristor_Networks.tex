...
\onslide<3->{$$f = \int_0^{h(f)v} p(t) dt$$
\centering
...
\onslide<2->{Switching a memristor: $f\to f+\frac{1}{N}$}

\onslide<3->{Increases current, $h(f)V\to h(f)V + \frac{h'(f)V}{N}$}

\onslide<4->{$$\frac{\int_{h(f)V}^{h(f)V + \frac{h'(f)V}{N}} p(t) }{\int_{h(f)V}^{\infty} p(t)} \approx \frac{p(h(f)V)h'(f)V}{(1-f)N}$$}

\onslide<5->{$$P(n) \to \frac{(\mu)^n}{n!} e^{-\mu}, \quad \mu = p(h(f)v)h'(f)v$$}
...
\onslide<2->{$$Y_n = 1 + Z_1 + Z_2 + ...+Z_n$$}
\onslide<3->{Generating Function $R_n(s)$
$$R_1(s) = sP(s)$$}
\onslide<4->{$$R_{n}(s) = sP(R_{n-1}(s))$$}
\onslide<5->{$R_{n+1}(s) < R_{n}(s)$ suggests, $$\rho(s) = sP(\rho(s))$$}

\end{frame}

\begin{frame}
\frametitle{Avalanche Size Distribution}
\onslide<1->{$$s = \rho e^\mu e^{-\mu \rho}$$}
\onslide<2->{$$\rho(s) = \sum_{n=1}^\infty s^n \frac{(\mu n)^{n-1}}{n!} e^{-\mu n}$$}
\onslide<3->{Borel Distribution $E(X) = \frac{1}{1-\mu}$ $Var(X)=\frac{\mu}{(1-\mu)^3}$}
